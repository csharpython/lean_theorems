...
theorem not_not_iff {P:Prop} : ¬¬P↔P := ⟨λa↦by_cases P _ (λp↦p) λp↦False.elim (a p),not_not⟩
@[simp]theorem not_not_not_iff {P:Prop} : ¬¬¬P↔¬P := ⟨λa p↦a λn↦n p,not_not⟩
theorem iff_not {P Q:Prop}(h:P ↔ Q) : ¬P↔¬Q := by rw[h]

theorem imp_of_not_or {P Q:Prop} : (¬P∨Q)→P→Q := λh p↦Or.elim h (λi↦False.elim (i p)) λq↦q
theorem not_imp {P Q:Prop} : (P→Q)↔(¬P∨Q) := ⟨λh↦by_cases P _ (λp↦Or.inr (h p)) λp↦Or.inl p,imp_of_not_or⟩
